...
/**
 * Размер тайла, от которого MapLibre отсчитывает зум.
 *
 * Растровые тайлы OSM приходят по 256 пикселей, но шкала зума в MapLibre GL
 * привязана к 512: на зуме z мир занимает 512·2^z пикселей. Если считать по
 * 256, зум выходит ровно на уровень крупнее нужного — вид оказывается вдвое
 * ближе, и точки, которые «помещались» по расчёту, уезжают за край карты.
 */
const TILE_SIZE = 512;

const DEFAULT_OPTIONS: Required<MapViewOptions> = {
  // Замерено по карточке карты в вёрстке: 678×358 при ширине окна 1440.
  widthPx: 678,
  heightPx: 358,
  padding: 0.25,
  minZoom: 2,
